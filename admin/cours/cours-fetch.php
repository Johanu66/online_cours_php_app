<?php
    ...
    $colonne = array("intitule_cours", "nom_classe", "nom_matiere", "date_debut_cours", "date_fin_cours", "prenom_personne", "fichier_1_cours", "fichier_2_cours", "fichier_3_cours", "fichier_4_cours", "statut_cours");

    $query = "";

    $output = array();

    $query .= "SELECT * FROM cours INNER JOIN classe ON id_classe_fk_cours=id_classe INNER JOIN matiere ON id_matiere_fk_cours=id_matiere INNER JOIN professeur ON id_professeur_fk_cours=id_professeur INNER JOIN personne ON id_personne_fk_professeur=id_personne WHERE del_cours='0' "; // changer
    ...
    foreach($result as $row)
    {
        ...
        /*$sub_array[] = '<a href="../../img/fichiers/'.$row["fichier_1_cours"].'">' . $row["fichier_1_cours"] . '</a>';
        $sub_array[] = '<a href="../../img/fichiers/'.$row["fichier_2_cours"].'">' . $row["fichier_2_cours"] . '</a>';
        $sub_array[] = '<a href="../../img/fichiers/'.$row["fichier_3_cours"].'">' . $row["fichier_3_cours"] . '</a>';
        $sub_array[] = '<a href="../../img/fichiers/'.$row["fichier_4_cours"].'">' . $row["fichier_4_cours"] . '</a>';*/

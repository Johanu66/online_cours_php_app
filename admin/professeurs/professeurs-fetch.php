<?php
    ...
    $colonne = array("photo_personne", "nom_personne", "prenom_personne", "sexe_personne", "nom_matiere", "tel_personne", "email_personne", "statut_personne");

    $query = "";

    $output = array();

    $query .= "SELECT * FROM professeur INNER JOIN personne ON id_personne_fk_professeur=id_personne INNER JOIN matiere ON id_matiere_fk_professeur=id_matiere WHERE del_personne='0' "; // changer
    ...
    foreach($result as $row)
    {
        ...
        <li><a href="#" class="dropdown-item view" id="'.$row["id_professeur"].'">Consulter</a></li>';
        if($_SESSION['id_type_compte'] <= 3){
            $actions .= "<li><a class='dropdown-item' href='professeurs-edit.php?id=".$row['id_professeur']."'>Modifier</a></li>";
        }
        if($_SESSION['id_type_compte'] <= 2){
            $actions .= "<li><a class='dropdown-item delete' href='#' id='".$row['id_professeur']."'>Supprimer</a></li>";
        }
        if($_SESSION['id_type_compte'] <= 1){
            if($row['statut_personne']=="Actif"){
                $actions .= "<li><a class='dropdown-item desactivation' href='#' id='".$row["id_professeur"]."'>Désactiver</a></li>";
            }
            else if($row['statut_personne']=="Inactif"){
                $actions .= "<li><a class='dropdown-item activation' href='#' id='".$row["id_professeur"]."'>Activer</a></li>";
            }
        }
        $actions .= '</ul>
        </div>';
    ...
    {
        $statement = $bdd->prepare("SELECT * FROM professeur INNER JOIN personne ON id_personne_fk_professeur=id_personne INNER JOIN matiere ON id_matiere_fk_professeur=id_matiere WHERE del_personne='0'"); // same query as above
        $statement->execute();

<?php
    ...
    if(isset($_POST['submit'])){
        if(!empty($_POST['matricule']) && !empty($_POST['id']) && !empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['email']) && !empty($_POST['sexe']) && !empty($_POST['classe']) && !empty($_POST['tel']) && !empty($_POST['date']) && !empty($_POST['password']) && !empty($_POST['password_confirm'])){
            ...
            if($password == $password_confirm){
                ...
                if($statement->rowCount() < 1){
                    ...
                    setcookie("success","L'étudiant ". $nom ." ". $prenom ." a été modifié.",time()+(86400*30),"/");
    ...
    if(isset($_GET['id']) && !empty($_GET['id'])){
        $statement = $bdd->prepare("SELECT * FROM etudiant INNER JOIN personne ON id_personne_fk_etudiant=id_personne INNER JOIN compte ON id_personne_fk_compte=id_personne INNER JOIN classe ON id_classe_fk_etudiant=id_classe WHERE del_personne='0' AND id_etudiant = ?");

<?php
    ...
    if(isset($_POST['submit'])){
        if(!empty($_POST['id']) && !empty($_POST['intitule']) && !empty($_POST['matiere']) && !empty($_POST['classe']) && !empty($_POST['professeur']) && !empty($_POST['date_debut'])){
            if(!isset($_POST['pas_de_fin']) && empty($_POST['date_fin'])){
                $alert_text = "Veuillez renseigner une date de fin.";
            }
            else{
                $id = htmlspecialchars($_POST['id']);
                $statement = $bdd->prepare("SELECT * FROM cours INNER JOIN classe ON id_classe_fk_cours=id_classe INNER JOIN matiere ON id_matiere_fk_cours=id_matiere INNER JOIN professeur ON id_professeur_fk_cours=id_professeur INNER JOIN personne ON id_personne_fk_professeur=id_personne WHERE del_cours='0' AND id_cours=?");
                $statement->execute(array($id));
                $old_value = $statement->fetch();
                $fichier1 = $old_value['fichier_1_cours'];
                $fichier2 = $old_value['fichier_2_cours'];
                $fichier3 = $old_value['fichier_3_cours'];
                $fichier4 = $old_value['fichier_4_cours'];
                ...
                setcookie("success","Le cours ". $intitule ." a été modifié.",time()+(86400*30),"/");
    ...
    if(isset($_GET['id']) && !empty($_GET['id'])){
        ...
        $old_value = $statement->fetch();
        $_POST['id'] = $old_value['id_cours'];
        $_POST['intitule'] = $old_value['intitule_cours'];
        $_POST['classe'] = $old_value['id_classe_fk_cours'];
        $_POST['professeur'] = $old_value['id_professeur_fk_cours'];
        $_POST['matiere'] = $old_value['id_matiere_fk_cours'];
        $_POST['date_debut'] = date("Y-m-d\TH:i", strtotime($old_value['date_debut_cours']));
        $_POST['notes'] = $old_value['notes_cours'];
        if(isset($old_value['date_fin_cours'])){
            $_POST['date_fin'] = date("Y-m-d\TH:i", strtotime($old_value['date_fin_cours']));
        }
        else{
            $_POST['pas_de_fin'] = true;
        }
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="product-payment-inner-st">
                                <ul id="myTabedu1" class="tab-review-design">
                                    <li class="active">Modification de cours</li>
                                </ul>
                                                                            ...
                                                                            <select name="professeur" id="professeur" class="form-control">
                                                                                ...
                                                                                <?php
                                                                                    if(isset($_POST['professeur'])){
                                                                                        echo(charger_professeur($_POST['professeur']));
                                                                                    }else{
                                                                                        echo(charger_professeur(null));
                                                                                    }
                                                                                ?>
                                                                            </select>
                                                                        </div>

<?php
    ...
    if(isset($_POST['submit'])){
        if(!empty($_POST['etudiant']) && !empty($_POST['motif'])){
            $etudiant = htmlspecialchars($_POST['etudiant']);
            $motif = htmlspecialchars($_POST['motif']);
            $now = new DateTime();
            insert("signale",[
                "motif_signale" => $motif,
                "id_etudiant_fk_signale" => $etudiant,
                "id_professeur_fk_signale" => get_id_prof_by_id_personne($_SESSION['id']),
                "statut_signale" => 'Non lu',
                "date_create_signale" => $now->format('Y-m-d H:i:s'),
                "user_create_signale" => $_SESSION['nom']." ".$_SESSION['prenom'],
                "date_last_modif_signale" => $now->format('Y-m-d H:i:s'),
                "user_last_modif_signale" => $_SESSION['nom']." ".$_SESSION['prenom']
            ]);
            $alert_title = "Effectué";
            $alert_text = "L'étudiant a été signalé.";
            $alert_type = "success";
        }
        else{
            $alert_title = "Échoué";
...
<html lang="fr">
    ...
    <body>
        <?php
            ...
            elseif(isset($alert_text)){
                ?>
                <script>
                    swal("<?php echo $alert_title ?>", "<?php echo $alert_text ?>", "<?php echo $alert_type ?>")
                </script>
                <?php
            }
        ?>
        ...
        <script>
            ...
            $(document).on('click', '.signaler', function(){
                var id = $(this).attr("id");
                var btn_action = "signaler";
                $.ajax({
                    url:"cours-action.php",
                    method:"POST",
                    ...
                    success:function(data)
                    {
                        ...
                        $('.modal-title_form').text("Signalement d'étudiant");
                        $('#form').html(data);
                    }
                })
            });
            ...
            $(document).on('click', '.view_students', function(){
                var id = $(this).attr("id");
                var btn_action = "liste des etudiants"
                $.ajax({
                    url:"cours-action.php",
                    method:"POST",
                    data:{id:id,btn_action:btn_action},
                    ...
                    {
                        $('#modal_view').modal('show');
                        $('.modal-title_view').text("Tous les étudiants");
                        $('#details').html(data);

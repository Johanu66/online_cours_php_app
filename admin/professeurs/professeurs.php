...
<html lang="fr">
    <head>
        ...
        <title>Professeurs | O-Class</title>
    ...
    <body>
                                    ...
                                    <div class="main-sparkline13-hd" style="display: flex; justify-content: space-between;margin-bottom: 40px;">
                                        ...
                                        <?php
                                            if($_SESSION['id_type_compte'] <= 4){
                                                ?>
                                                <a href="professeurs-add.php"><button class="btn btn-primary mr-2">Ajouter un nouveau professeur</button></a>
                                                <?php
                                            }
                                        ?>
                                    </div>
        ...
        <script>
            ...
            console.log($('#table'));
            var professeursDataTable = $('#table').DataTable({
                "processing":true,
                "serverSide":true,
                "order":[],
                "ajax":{
                    url:"professeurs-fetch.php", //changer
                    type:"POST"
                },
                "columnDefs":[
                    {
            ...
            $(document).on('click', '.view', function(){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        $('.modal-title_view').text("Détails sur le professeur");
                        $('#details').html(data);
            ...
            $(document).on('click', '.activation', function(){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        professeursDataTable.ajax.reload();
                    }
                })
            });
            ...
            $(document).on('click', '.delete', function(){
                var id = $(this).attr('id');
                swal({
                    title: 'Suppression d\'professeur',
                    text: 'Etes-vous sûre de vouloir supprimer ce professeur ?',
                    icon: 'warning',
                    buttons: true,
                    dangerMode: false,
                })
